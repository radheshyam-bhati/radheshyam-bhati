...
USER_DATA=$(curl -s "https://api.github.com/users/$USERNAME" 2>/dev/null)
REPOS_DATA=$(curl -s "https://api.github.com/users/$USERNAME/repos?per_page=100&type=public" 2>/dev/null)
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 160" width="100%" height="auto">
  ...
  <!-- Panel background -->
  <rect x="0" y="0" width="600" height="160" rx="6" fill="url(#meterBg)" stroke="#2A2F38" stroke-width="1.5"/>
  <rect x="3" y="3" width="594" height="154" rx="4" fill="none" stroke="#8B7355" stroke-width="0.5" opacity="0.3"/>

  <!-- Panel title -->
  <rect x="200" y="6" width="200" height="16" rx="2" fill="#0A0B0C" stroke="#8B7355" stroke-width="0.5"/>
  <text x="300" y="18" font-family="'Courier New', monospace" font-size="8" fill="#C4A35A" text-anchor="middle" letter-spacing="4" opacity="0.8">LIVE METER PANEL</text>

  <!-- Meter 1: Lines Open (Repos) -->
  <g transform="translate(20, 30)">
    <rect x="0" y="0" width="130" height="55" rx="3" fill="#0A0B0C" stroke="#2A2F38" stroke-width="0.5"/>
    <text x="8" y="14" font-family="monospace" font-size="7" fill="#8B7355" letter-spacing="1">LINES OPEN</text>
    <!-- Bar background -->
    <rect x="8" y="20" width="110" height="8" rx="4" fill="#1A1D23" stroke="#2A2F38" stroke-width="0.5"/>
    <!-- Bar value (capped at 50 for visual scale) -->
    <rect x="8" y="20" width="$(( PUBLIC_REPOS > 50 ? 110 : PUBLIC_REPOS * 2 ))" height="8" rx="4" fill="url(#meterBar)" opacity="0.9"/>
    <text x="8" y="44" font-family="monospace" font-size="18" fill="#D45500" filter="url(#glow)">$(printf "%02d" $PUBLIC_REPOS)</text>
    <text x="100" y="44" font-family="monospace" font-size="7" fill="#5A4A35" text-anchor="end">repos</text>
  </g>
  ...
  <g transform="translate(165, 30)">
    ...
    <rect x="8" y="20" width="110" height="8" rx="4" fill="#1A1D23" stroke="#2A2F38" stroke-width="0.5"/>
    <rect x="8" y="20" width="$(( TOTAL_STARS > 50 ? 110 : TOTAL_STARS * 2 ))" height="8" rx="4" fill="url(#meterBarGold)" opacity="0.9"/>
    <text x="8" y="44" font-family="monospace" font-size="18" fill="#C4A35A" filter="url(#glow)">$(printf "%02d" $TOTAL_STARS)</text>
    <text x="100" y="44" font-family="monospace" font-size="7" fill="#5A4A35" text-anchor="end">stars</text>
  </g>
  ...
  <g transform="translate(310, 30)">
    ...
    <rect x="8" y="20" width="110" height="8" rx="4" fill="#1A1D23" stroke="#2A2F38" stroke-width="0.5"/>
    <rect x="8" y="20" width="$(( FOLLOWERS > 50 ? 110 : FOLLOWERS * 2 ))" height="8" rx="4" fill="url(#meterBarBlue)" opacity="0.9"/>
    <text x="8" y="44" font-family="monospace" font-size="18" fill="#4A90D9" filter="url(#glow)">$(printf "%02d" $FOLLOWERS)</text>
    <text x="100" y="44" font-family="monospace" font-size="7" fill="#5A4A35" text-anchor="end">followers</text>
  </g>
  ...
  <g transform="translate(455, 30)">
    ...
    <rect x="8" y="20" width="110" height="8" rx="4" fill="#1A1D23" stroke="#2A2F38" stroke-width="0.5"/>
    <rect x="8" y="20" width="$(( TOTAL_FORKS > 50 ? 110 : TOTAL_FORKS * 2 ))" height="8" rx="4" fill="url(#meterBarGreen)" opacity="0.9"/>
    <text x="8" y="44" font-family="monospace" font-size="18" fill="#00A86B" filter="url(#glow)">$(printf "%02d" $TOTAL_FORKS)</text>
    <text x="100" y="44" font-family="monospace" font-size="7" fill="#5A4A35" text-anchor="end">forks</text>
  </g>

  <!-- Bottom status bar -->
  <rect x="20" y="95" width="560" height="1" fill="#2A2F38" opacity="0.5"/>
  ...
  <g transform="translate(20, 122)">
    ...
    <text x="65" y="10" font-family="monospace" font-size="7" fill="#E8D5B7">${LANGUAGES:0:60}</text>
  </g>

  <!-- Pulse indicators -->
  <circle cx="565" cy="108" r="3" fill="#00A86B" filter="url(#glow)"/>
  <text x="555" y="112" font-family="monospace" font-size="6" fill="#5A4A35" text-anchor="end">LIVE</text>
